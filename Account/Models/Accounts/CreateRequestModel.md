...
Le modèle de requête de création définit les paramètres des requêtes POST entrantes vers la route **/accounts**, il est attaché à la route en le définissant comme paramètre de la méthode d'action Create du accounts controller. Lorsqu'une requête HTTP POST est reçue par la route, les données du corps sont liées à une instance de la classe **CreateRequest**, validées et transmises à la méthode.
