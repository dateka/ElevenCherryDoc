...
Le modèle de demande de registre définit les paramètres des demandes POST entrantes vers la route **/accounts/register**, il est attaché à la route en le définissant comme paramètre de la méthode d'action **Register** du **account controller**. Lorsqu'une requête HTTP POST est reçue par la route, les données du corps sont liées à une instance de la classe **RegisterRequest**, validées et transmises à la méthode.
