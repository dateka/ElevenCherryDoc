...
Le modèle de demande de réinitialisation de mot de passe définit les paramètres des demandes POST entrantes vers la route **/accounts/reset-password**, il est attaché à la route en le définissant comme paramètre de la méthode d'action **ResetPassword** du **account controller**. Lorsqu'une requête HTTP POST est reçue par la route, les données du corps sont liées à une instance de la classe **ResetPassword**, validées et transmises à la méthode.
