...
Le modèle de demande de mise à jour définit les paramètres des demandes PUT entrantes vers la route **/accounts/{id:int}**, il est attaché à la route en le définissant comme paramètre de la méthode d'action **Update** du **account controller**. Lorsqu'une requête HTTP PUT est reçue par la route, les données du corps sont liées à une instance de la classe **UpdateRequest**, validées et transmises à la méthode.
